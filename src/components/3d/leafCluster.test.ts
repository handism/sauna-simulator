...
import {
  LEAF_CLUSTER_SIZE, applyLeafCluster, createLeafClusterTexture, leafClusterCoverage, leafClusterMips, leafClusterOf,
} from './leafCluster';

const share = (alpha: Float32Array | Uint8Array, cutoff: number) => alpha.filter(value => value >= cutoff).length / alpha.length;

describe('woodland leaf cluster cards', () => {
  it('reads only a positive integer leaf count from glTF extras', () => {
    const material = (extras: unknown) => Object.assign(new THREE.MeshStandardMaterial(), { userData: { suiLeafCluster: extras } });
  ...
  it('draws the same leaves every time and covers about half of the card', () => {
    ...
    expect(mean).toBeGreaterThan(.42);
  ...
  it('keeps the alpha-tested coverage in the mip levels that distant cards sample', () => {
    ...
    const base = share(levels[0], .5);
    for (const level of levels.slice(1, 5)) expect(Math.abs(share(level, .5) - base)).toBeLessThan(.05);
    ...
    for (let width = 32; width >= 4; width /= 2) {
      box = Float32Array.from({ length: width * width }, (_, i) => {
        ...
        return [0, 1, width * 2, width * 2 + 1].reduce((sum, offset) => sum + box[2 * y * width * 2 + 2 * x + offset], 0) / 4;
      });
    }
    expect(share(box, .5)).toBeLessThan(base - .1);
  });
  ...
  it('uploads explicit mips and cuts out cards with alpha to coverage', () => {
    ...
    expect(material).toMatchObject({ alphaMap: texture, alphaTest: .5, alphaToCoverage: true, transparent: false });
