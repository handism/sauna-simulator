...
describe('water surface', () => {
  it('blends only the Fresnel reflection of the reflection probes over the pool', () => {
    const probes = { ...createIrradianceUniforms(), ...createReflectionUniforms() };
    ...
    expect(surface.material.uniforms.suiReflectionCourtyard).toBe(probes.suiReflectionCourtyard);
