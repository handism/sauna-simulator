...
export const FOLIAGE_TRANSMISSION = .6;
...
export function patchFoliageShader(fragmentShader: string): string {
  ...
  if (!fragmentShader.includes(PARS_INCLUDE) || !fragmentShader.includes(BEGIN_INCLUDE) || !begin.includes(HEMISPHERE_TARGET)) {
  ...
  return fragmentShader
    .replace(PARS_INCLUDE, `${PARS_INCLUDE}
...
#define RE_Direct RE_Direct_Foliage`)
    .replace(BEGIN_INCLUDE, begin.replace(HEMISPHERE_TARGET, `${HEMISPHERE_TARGET}
...
export function applyFoliageTransmission(material: THREE.MeshStandardMaterial) {
  ...
  material.onBeforeCompile = (shader) => { shader.fragmentShader = patchFoliageShader(shader.fragmentShader); };
