...
vi.mock('three', async importOriginal => {
  ...
  return { ...actual, WebGLRenderer: class {
    ...
    setPixelRatio = vi.fn(); getPixelRatio = () => 1;
    setSize = vi.fn(); render = vi.fn(); setAnimationLoop = vi.fn(); dispose = vi.fn();
...
function mountScene() {
  ...
  const onReady = vi.fn(); const onError = vi.fn();
  const view = render(<SaunaScene audio={audio} quality="standard" stage="sauna" lightingMode="day"
    loylyEvents={new EventTarget()} onReady={onReady} onError={onError} />);
...
beforeEach(() => {
  vi.useFakeTimers();
  mocks.renderers.length = 0; mocks.parse.mockReset();
  ...
  vi.stubGlobal('fetch', vi.fn(async (url: string) => ({
    ok: true, json: async () => definition, arrayBuffer: async () => new ArrayBuffer(0), url,
  })));
});
...
describe('3D scene load and teardown', () => {
  ...
  it.each(['timeout', 'context loss', 'unmount'])('disposes a model resolved after %s without restarting rendering', async reason => {
    const pending = deferred<any>(); mocks.parse.mockReturnValue(pending.promise);
    const loaded = model(); const view = mountScene(); await flush();
  ...
  it('stops a running scene on context loss, reports once, and releases it on exit', async () => {
    const loaded = model(); mocks.parse.mockResolvedValue(loaded);
    const view = mountScene(); await flush();
