...
vi.mock('./3d/SaunaScene', () => ({ default: function MockScene({stage, quality, lightingMode, loylyEvents, onReady, onError}: SceneProps) {
  ...
  return <button data-quality={quality} data-lighting={lightingMode} onClick={onError}>simulate context loss</button>;
}}));
...
describe('3D mode lifecycle', () => {
  ...
  it('keeps lighting across stages and mode switches without remounting the active scene', async () => {
    ...
    fireEvent.click(screen.getByRole('button', {name:'2Dに切り替え'}));
    fireEvent.click(screen.getByRole('button', {name:'3Dを試す'}));
    expect(await screen.findByRole('button', {name:'simulate context loss'})).toHaveAttribute('data-lighting', 'evening');
    view.unmount();
    render(<SceneMode audio={audio} stage="sauna" loylyEvents={events} />);
