...
import { useSaunaSession, Stage } from './hooks/useSaunaSession';
  ...
  return (
    <div className={`app-container ${isUiHidden ? 'ui-hidden' : ''}`} style={{ background: '#000' }}>
      ...
      {BACKGROUNDS.map(({ stage: s, gradient, image }) => (
        <div key={s} className="app-bg-layer" style={{ opacity: stage === s ? 1 : 0, backgroundImage: `linear-gradient(${gradient}), url(${import.meta.env.BASE_URL}${image})` }} />
      ))}
