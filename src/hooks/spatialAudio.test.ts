...
const node = () => ({ connect: vi.fn(function (target) { return target; }), gain: param() });
const pose: SpatialPose = { position: [1, 2, 3], forward: [0, 0, -1], up: [0, 1, 0], stove: [-2, 1, 0], water: [3, 1, 2] };
function setup() {
  const listener = Object.fromEntries(['position', 'forward', 'up'].flatMap(prefix => ['X', 'Y', 'Z'].map(axis => [prefix + axis, param()])));
  const ctx = { currentTime: 4, listener, createGain: vi.fn(node), createPanner: vi.fn(() => ({ ...node(), positionX: param(), positionY: param(), positionZ: param() })) };
  const master = node();
...
describe('spatial sound routing', () => {
  ...
  it('crossfades back to the original level on 2D/failure and tolerates repeated toggles', () => {
    ...
    for (const index of [1, 4]) {
      expect(gains[index].gain.setTargetAtTime).toHaveBeenCalledExactlyOnceWith(0, 4, .08);
      expect(gains[index + 1].gain.setTargetAtTime).toHaveBeenCalledExactlyOnceWith(1, 4, .08);
    }
    spatial.update(null);
    for (const index of [1, 4]) {
      expect(gains[index].gain.setTargetAtTime).toHaveBeenLastCalledWith(1, 4, .08);
      expect(gains[index + 1].gain.setTargetAtTime).toHaveBeenLastCalledWith(0, 4, .08);
    }
    for (let i = 0; i < 20; i++) { spatial.update(pose); spatial.update(null); }
    expect(ctx.createPanner).toHaveBeenCalledTimes(2);
    expect(ctx.createGain).toHaveBeenCalledTimes(6);
  });
});
