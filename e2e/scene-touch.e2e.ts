...
test('touch look survives a second finger and cancellation; controls remain tappable', async ({ page }, info) => {
  ...
  const touch = (type: 'touchStart' | 'touchMove' | 'touchEnd' | 'touchCancel', touchPoints: { id: number; x: number; y: number }[]) =>
  ...
  await touch('touchStart', [{ id: 1, x: 150, y: 420 }, { id: 2, x: 280, y: 420 }]);
  await touch('touchMove', [{ id: 1, x: 150, y: 420 }, { id: 2, x: 300, y: 420 }]);
  ...
  await info.attach('touch-validation', {
    body: JSON.stringify({ browser: page.context().browser()?.version(), viewport: page.viewportSize(), input: 'CDP touch emulation', errors }, null, 2),
