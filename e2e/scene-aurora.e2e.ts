...
test('3D lighting replaces the aurora only while the scene is ready', async ({ page, browser }, info) => {
  ...
  for (const lighting of ['day', 'evening']) {
    ...
    const legacy = await page.addStyleTag({
      content:
        '.aurora-container { display: block !important; opacity: .6 !important; transition: none !important; } .aurora-blob { animation: none !important; }',
    });
    ...
    body: JSON.stringify(
      {
        ...
        reducedMotion: true,
        muted: true,
        comparison: 'Legacy overlay reconstructed at fixed opacity .6; model, camera and lighting unchanged.',
        errors,
      },
      null,
      2,
