...
const reference: { input_sha256: string; cameras: Camera[] } =
  JSON.parse(readFileSync(new URL('./fixtures/cycles-cameras.json', import.meta.url), 'utf8'));
...
test('capture the Cycles review cameras in the browser scene', async ({ page, browser }, info) => {
  ...
  for (const views of passes) {
    ...
    for (const [stage, next] of stages) {
      ...
      for (const lighting of ['day', 'evening']) {
        ...
        const hide = await page.addStyleTag({ content: '.scene-mode-controls, .mute-btn { visibility: hidden !important; transition: none !important; }' });
        ...
        const render = reference.cameras.find(other => other.camera === camera.camera && other.lighting === lighting)?.render ?? null;
        samples.push({ file, stage, lighting, render, camera: camera.camera,
  ...
  await info.attach('cycles-compare', { contentType: 'application/json', body: JSON.stringify({
    browser: browser.version(), viewport: page.viewportSize(), deviceScaleFactor: 1, quality: 'standard',
    reducedMotion: true, muted: true, cameraReference: reference.input_sha256, hashes, errors, samples,
