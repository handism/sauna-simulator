...
React 19 + Vite のシングルページアプリ。TypeScript は使用していない（`.js` / `.jsx`）。

### ステージ遷移フロー

`App.jsx` がアプリ全体の状態を管理する。ステージは `start → sauna → water → totonou` の順に進み、`totonou` から `sauna` に戻るループ構造になっている。
...
- 背景画像は `App.jsx` 内で3枚のレイヤーとして常時レンダリングされており、`opacity` の切り替えでクロスフェードを実現している（`public/sauna_bg.png`, `water_bg.png`, `totonou_bg.png`）

### オーディオ

`src/hooks/useAudioEngine.js` にカプセル化されており、**外部音声ファイルは一切使用しない**。すべて Web Audio API でプロシージャル生成している。
...
- `audio` オブジェクト（`useAudioEngine` の戻り値）は `App.jsx` で生成し、必要なコンポーネントに props で渡す
