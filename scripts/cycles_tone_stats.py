...
for c in cams:
    ...
    Lc, Cc, Hc = lab(root/'blender/renders'/c['render']); Lw, Cw, Hw = lab(web[0])
    green = lambda H, C: (H > 90) & (H < 150) & (C > 12)
    gc, gw = green(Hc, Cc), green(Hw, Cw)
    rows.append(dict(render=c['render'], light=light,
...
print('mean dL*', d[:, 0].mean().round(2), 'mean |dL*|', np.abs(d[:, 0]).mean().round(2), 'mean dC*', d[:, 1].mean().round(2))
P = np.array([np.abs(r['Lp'][1] - r['Lp'][0]) for r in rows])
G = np.array([abs(r['greenC'][1] - r['greenC'][0]) for r in rows])
print('percentile |dL| L10/L50/L90', P.mean(0).round(2), 'all', P.mean().round(2), '| green |dC|', np.nanmean(G).round(2))
