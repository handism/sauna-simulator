...
assert(input && output && reportPath, 'Usage: node scripts/compress_web_glb.mjs input.glb output.glb report.json [float-bits]');
...
for (const [index, view] of model.bufferViews.entries()) {
  ...
  const encodedInput = filter === 'EXPONENTIAL'
    ? MeshoptEncoder.encodeFilterExp(new Float32Array(bytes.buffer, bytes.byteOffset, bytes.length / 4), accessor.count, stride, bits, 'Separate')
    : bytes;
  ...
  if (filter === 'NONE') {
    ...
    for (const [key, operation] of [['min', Math.min], ['max', Math.max]]) {
  ...
  view.extensions = { EXT_meshopt_compression: {
    buffer: 0, byteOffset: append(Buffer.from(compressed)), byteLength: compressed.length,
    byteStride: stride, count: accessor.count, mode, filter,
  } };
  verification.compressedViews++;
}
model.buffers = [{ byteLength }, { byteLength: fallbackLength, extensions: { EXT_meshopt_compression: { fallback: true } } }];
for (const key of ['extensionsUsed', 'extensionsRequired']) model[key] = [...(model[key] ?? []), 'EXT_meshopt_compression'];
...
writeFileSync(reportPath, JSON.stringify({
  codec: 'meshoptimizer 1.1.1', extension: 'EXT_meshopt_compression', version: 0,
  floatBits: bits, inputBytes: source.length, outputBytes: result.length,
  inputSha256: hash(source), outputSha256: hash(result), verification,
}, null, 2) + '\n');
